...
class ContentBasedFiltering:
    ...
    def get_recommendations_from_user_profile(user_id: int, top_k: int = 20, previous_job_ids: List[str] = None) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            # ユーザーの質問回答を取得
            cur.execute("""
                SELECT dq.question_key, dq.category, uqr.normalized_response, uqr.response_text, uqr.question_key as user_question_key
            ...
            for resp in responses:
                q_key = resp.get('question_key') or resp.get('user_question_key')
                print(f"  📝 {q_key}: {resp.get('response_text') or resp.get('normalized_response')}")
            ...
            for resp in responses:
                # question_keyはdynamic_questionsから取得、NULLの場合はuser_question_keyを使用
                original_key = resp.get('question_key') or resp.get('user_question_key')
                
                if not original_key:
                    print(f"  ⚠️ Skipping response with no question_key")
                    continue
                
                # 自動正規化
                key = normalize_key(original_key)
                value = (resp.get('normalized_response') or resp.get('response_text') or '').strip().lower()
                
                if not value:
                    print(f"  ⚠️ Skipping response with no value for {original_key}")
                    continue
