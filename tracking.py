...
class QuestionResponseManager:
    ...
    def save_response(
        ...
        Args:
            user_id: ユーザーID
            question_id: 質問ID（オプション）
            question_key: 質問キー（オプション）
            category: カテゴリ（オプション）
        ...
        try:
            ...
            # question_id または question_key のどちらかは必須
            if question_id:
                # UPSERT (既に回答がある場合は更新)
                cur.execute("""
                ...
                # question_key のみの場合（question_idなし）
                # categoryは記録しない（テーブルにカラムがないため）
                cur.execute("""
                    INSERT INTO user_question_responses 
                ...
                raise ValueError("question_id または question_key のいずれかが必要です")

            conn.commit()
            cur.close()
            conn.close()
            
            # categoryは受け取るが使用しない（互換性のため）
            if category:
    ...
    def get_user_responses(user_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # デバッグログ追加
            print(f"🔍 DEBUG: Found {len(responses)} responses for user {user_id}")
